        ...
        >
          <div className="space-y-2.5 max-w-md mx-auto">
            <span className="text-[9px] font-mono text-stone-400 uppercase tracking-widest font-bold block">
              MyArtNotes — {locale === "en" ? "GUEST PORTAL" : "PORTAL DO VISITANTE"}
            </span>
            <h4 className="font-serif text-lg font-semibold text-stone-900 leading-tight">
              {locale === "en" ? "Welcome to your Atelier Notebook" : "Boas-vindas ao seu Caderno de Ateliê"}
            </h4>
            <p className="text-xs text-stone-550 leading-relaxed font-sans">
              {isGuest ? (
                locale === "en" 
                  ? "This is a digital sanctuary for sifting, documenting, and cross-referencing thoughts in your creative practice. Sign in to start your private research notebook, record logs, and generate semantic constellations." 
                  : "Este é um santuário digital para filtrar, documentar e cruzar pensamentos semeados em sua prática criativa. Faça login para iniciar seu caderno de pesquisa privado, registrar experimentos e gerar constelações semânticas."
              ) : (
                locale === "en" 
          ...
          {isGuest && onSignIn ? (
            <button
              type="button"
              onClick={onSignIn}
              className="px-5 py-2.5 bg-stone-900 hover:bg-stone-850 text-white font-mono text-xs uppercase tracking-wider rounded font-bold cursor-pointer transition-colors shadow-xs inline-flex items-center gap-2"
            >
              ...
              <span>{locale === "en" ? "Sign In with Google" : "Acessar com o Google"}</span>
            </button>
          ) : (
            <button
              type="button"
